// Ocorrencias management module
window.ocorrenciasModule = (function() {
    let ocorrencias = [];
    let editingId = null;

    function loadOcorrencias() {
        const stored = localStorage.getItem('motorista_padrao_ocorrencias');
        ocorrencias = stored ? JSON.parse(stored) : [];
        renderOcorrencias();
        return ocorrencias;
    }

    function saveOcorrencias() {
        localStorage.setItem('motorista_padrao_ocorrencias', JSON.stringify(ocorrencias));
    }
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; gap: 8px;">
                        <button class="btn-primary btn-sm btn-icon" title="Editar" onclick="window.ocorrenciasModule.edit(${oc.id})">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-danger btn-sm btn-icon" title="Excluir" onclick="window.ocorrenciasModule.delete(${oc.id})">
    ...
    function saveOcorrencia(event) {
        ...
        if (editingId) {
            const index = ocorrencias.findIndex(o => o.id === editingId);
            if (index !== -1) {
                ocorrenciaData.id = editingId;
                ocorrencias[index] = ocorrenciaData;
                utils.showAlert('Ocorrência atualizada com sucesso!', 'success');
            }
        } else {
            ocorrenciaData.id = Date.now();
            ocorrencias.push(ocorrenciaData);
            utils.showAlert('Ocorrência registrada com sucesso!', 'success');
        }

        saveOcorrencias();
        renderOcorrencias();
        closeModal();
        updateSystemViews(); // Retira do Hall da Fama instantaneamente
    }

    function deleteOcorrencia(id) {
        if (confirm('Tem certeza que deseja excluir este registro de ocorrência?')) {
            ocorrencias = ocorrencias.filter(o => o.id !== id);
            saveOcorrencias();
            renderOcorrencias();
            utils.showAlert('Ocorrência excluída com sucesso!', 'success');
            updateSystemViews(); // Devolve ao Hall da Fama instantaneamente (se for o caso)
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const form = document.getElementById('ocorrencia-form');
        if (form) {
            form.addEventListener('submit', saveOcorrencia);
        }
    });
