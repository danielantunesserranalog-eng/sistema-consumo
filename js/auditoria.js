...
window.auditoriaModule = (function() {
    function render() {
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
                    <div>
                        ...
                        <p style="color: #94a3b8; font-size: 0.85rem; line-height: 1.6;">
                            ...
                            <strong>Melhores Marcas (Usadas como base 100%):</strong> KM/L = ${utils.formatNumber(maxKML)} | Distância = ${utils.formatNumber(maxDistance, 0)} km<br>
                            <strong>Trava de Qualificação:</strong> Mínimo de ${DISTANCIA_MINIMA_QUALIFICACAO} km rodados no mês.
                        </p>
                    </div>
                    <div style="min-width: 250px;">
                        <input type="text" id="gerencial-search" onkeyup="window.filterGerencial()" placeholder="Buscar motorista (Nome)..." class="form-control filter-input" style="width: 100%; background: #0f172a;">
                    </div>
                </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>Motorista</th>
                                <th>KM/L Real</th>
                                <th>Pts Consumo (Máx 700)</th>
                                <th>KM Real</th>
                                <th>Pts Distância (Máx 300)</th>
                                <th>Índice Final</th>
                            </tr>
                        </thead>
                        <tbody>
                ...
                <tr class="gerencial-row" data-name="${escapeHtml(d.name)}">
                    ...
                    <td>
                        <div style="display: flex; flex-direction: column;">
                            <span style="color: #38bdf8; font-weight: bold;">${d.ptsKml} pts</span>
                            <span style="font-size: 0.7rem; color: #64748b;">Atingiu ${kmlPerc}% da melhor média</span>
                        </div>
                    </td>
                    <td>${utils.formatNumber(d.total_distance, 0)}</td>
                    <td>
                        <div style="display: flex; flex-direction: column;">
                            <span style="color: #fbbf24; font-weight: bold;">${d.ptsDist} pts</span>
                            <span style="font-size: 0.7rem; color: #64748b;">Atingiu ${distPerc}% da maior distância</span>
                        </div>
                    </td>
        ...
        if (disqualifiedDrivers.length > 0) {
            detailsHtml += `
                <tr>
                    <td colspan="7" style="background: rgba(239, 68, 68, 0.1); color: #f87171; text-align: center; font-weight: bold; padding: 12px; font-size: 0.85rem;">
                        MOTORISTAS DESCLASSIFICADOS NESTE MÊS
                    </td>
                </tr>
            `;
            ...
            disqualifiedDrivers.sort((a,b) => b.total_distance - a.total_distance).forEach(d => {
                let reason = "";
                if ((d.total_distance || 0) < DISTANCIA_MINIMA_QUALIFICACAO) {
                    reason = `Faltou KM (Fez apenas ${utils.formatNumber(d.total_distance, 0)} km)`;
                } else {
                    reason = "Teve Ocorrência Registrada no Mês";
                }
                ...
                detailsHtml += `
                    <tr class="gerencial-row" data-name="${escapeHtml(d.name)}" style="opacity: 0.7;">
                        ...
                        <td>${utils.formatNumber(d.avg_economy)}</td>
                        <td style="color: #64748b;">Zerado</td>
                        <td>${utils.formatNumber(d.total_distance, 0)}</td>
                        <td style="color: #64748b;">Zerado</td>
