...
async function loadDashboardData() {
    ...
    try {
        ...
        renderAlertsTable();
    } catch (error) { console.error('Erro:', error); showDashboardError(); }
}
...
function calculateMetrics(viagens) {
    ...
    dashboardData.trucks.forEach(t => { if (t.avgConsumption < 1.80 && t.avgConsumption > 0) dashboardData.alerts.push({ type: 'consumption', placa: t.plate, value: t.avgConsumption.toFixed(2) }); });
    
    const truckTrips = new Map();
    viagens.forEach(v => { const plate = v.placa || 'Desconhecido'; truckTrips.set(plate, (truckTrips.get(plate) || 0) + 1); });
    for (const [plate, count] of truckTrips) { const avg = count / daysDiff; if (avg < 2) dashboardData.alerts.push({ type: 'productivity', placa: plate, value: avg.toFixed(1) }); }
    
    updateStatsCards();
}

function updateStatsCards() {
    const el = (id, val) => { const e = document.getElementById(id); if (e) { e.innerHTML = val; if (id === 'avgConsumption' && dashboardData.avgConsumption < 1.80 && dashboardData.avgConsumption > 0) e.style.color = '#dc2626'; else e.style.color = '#1e293b'; } };
...
function renderAlertsTable() {
    ...
    if (!tbody) return;
    if (dashboardData.alerts.length === 0) { tbody.innerHTML = '<tr><td colspan="5">Nenhum alerta. Parabéns!</td></tr>'; return; }
...
// ==================== GRÁFICOS ====================
function renderDashboardCharts() { renderDriverChart(); renderTruckChart(); renderTimeDistributionChart(); }
...
function renderDriverChart() {
    ...
    driverChart = new Chart(canvas.getContext('2d'), { type: 'bar', data: { labels: topDrivers.map(d => d.name.length > 15 ? d.name.substring(0, 15) + '...' : d.name), datasets: [{ label: 'KM/L', data: topDrivers.map(d => d.avgConsumption), backgroundColor: topDrivers.map(d => d.avgConsumption < 1.80 ? '#ef4444' : '#3b82f6'), borderRadius: 8 }] }, options: { responsive: true, maintainAspectRatio: true, scales: { y: { beginAtZero: true, title: { display: true, text: 'KM/L' } } } } });
}
...
function renderTruckChart() {
    ...
    truckChart = new Chart(canvas.getContext('2d'), { type: 'bar', data: { labels: topTrucks.map(t => t.plate), datasets: [{ label: 'KM/L', data: topTrucks.map(t => t.avgConsumption), backgroundColor: topTrucks.map(t => t.avgConsumption < 1.80 ? '#ef4444' : '#3b82f6'), borderRadius: 8 }] }, options: { responsive: true, maintainAspectRatio: true, scales: { y: { beginAtZero: true, title: { display: true, text: 'KM/L' } } } } });
}
...
async function renderTimeDistributionChart() {
    ...
    try {
        ...
        timeChart = new Chart(canvas.getContext('2d'), { type: 'doughnut', data: { labels: ['Tempo de Campo', 'Tempo de Fábrica'], datasets: [{ data: [campo, fabrica], backgroundColor: ['#3b82f6', '#10b981'], borderWidth: 0 }] }, options: { responsive: true, plugins: { tooltip: { callbacks: { label: (ctx) => { const total = campo + fabrica; const pct = total > 0 ? ((ctx.raw / total) * 100).toFixed(1) : 0; return `${ctx.label}: ${(ctx.raw / 3600).toFixed(1)}h (${pct}%)`; } } } } } });
...
function parseInterval(interval) {
    ...
    if (typeof interval === 'string') {
        ...
        let sec = 0;
        const h = interval.match(/(\d+) hours?/);
        const m = interval.match(/(\d+) minutes?/);
        const s = interval.match(/(\d+) seconds?/);
...
async function loadEvolutionData(type, value) {
    ...
    if (evolutionDataCache.has(cacheKey)) { const c = evolutionDataCache.get(cacheKey); renderEvolutionChart(c.labels, c.data, type, value); return; }
    try {
...
function renderEvolutionChart(labels, data, type, value) {
    ...
    if (!labels.length) { evolutionChart = new Chart(canvas.getContext('2d'), { type: 'line', data: { labels: ['Sem dados'], datasets: [{ label, data: [0], borderColor: '#94a3b8' }] } }); return; }
    evolutionChart = new Chart(canvas.getContext('2d'), { type: 'line', data: { labels, datasets: [{ label, data, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', borderWidth: 3, fill: true, tension: 0.3, pointRadius: 4 }] }, options: { responsive: true, scales: { y: { beginAtZero: true, title: { display: true, text: 'KM/L' } } } } });
}

// ==================== IMPORTAÇÃO OTIMIZADA (RÁPIDA) ====================
function initImportModule() {
    ...
    uploadArea.addEventListener('drop', async (e) => { e.preventDefault(); uploadArea.style.borderColor = '#cbd5e1'; const files = Array.from(e.dataTransfer.files).filter(f => f.name.endsWith('.csv')); if (files.length) await processFiles(files); else showImportStatus('error', 'Envie apenas arquivos CSV.'); });
    if (fileInput) fileInput.addEventListener('change', async (e) => { const files = Array.from(e.target.files); if (files.length) await processFiles(files); fileInput.value = ''; });
}
...
async function processFiles(files) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (!data.length) { addToImportLog(`⚠️ Nenhum dado válido`, 'warning'); continue; }
            ...
            addToImportLog(`✅ ${file.name}: ${inserted} inseridas, ${updated} atualizadas, ${errors} erros (${elapsed}s)`, 'success');
        } catch (error) { addToImportLog(`❌ Erro: ${error.message}`, 'error'); importStats.errors++; }
    }
...
function parseCSVFast(file) {
    return new Promise((resolve, reject) => {
        ...
        Papa.parse(file, {
            ...
            delimiter: ';',
            skipEmptyLines: true,
            chunkSize: 1024 * 1024, // 1MB chunks para processamento rápido
            step: (row) => {
...
function mapRowFast(row) {
    try {
        ...
        const inicio = parseDate(row['Início']);
        const fim = parseDate(row['Fim']);
        const placa = row['Identificador/Placa'] || '';
        if (!inicio || !fim || !placa) return null;
        return {
            placa: String(placa).trim(),
            motorista: String(row['Motorista'] || 'Não informado').trim(),
            inicio, fim,
            local_inicial: String(row['Local inicial'] || '').trim(),
            local_final: String(row['Local final'] || '').trim(),
            distancia_km: parseFloat(String(row['Distância (Km)'] || '0').replace(',', '.')),
            km_l: parseFloat(String(row['Km/l'] || '0').replace(',', '.')),
            tempo_conducao_seconds: parseDuration(row['Tempo de Condução']),
            tempo_parado_seconds: parseDuration(row['Tempo Parado'])
        };
    } catch (e) { return null; }
}
...
async function batchInsertSupabase(viagens, batchSize = 500) {
    ...
    for (let i = 0; i < viagens.length; i += batchSize) {
        ...
        // Verifica duplicatas em lote (mais rápido)
...
function clearImportLog() { const logDiv = document.getElementById('importLog'); if (logDiv) logDiv.innerHTML = ''; }
...
function showEmptyDashboard() {
    ...
    const tbody = document.getElementById('alertsTableBody');
    if (tbody) tbody.innerHTML = '<tr><td colspan="5">Nenhum dado. Importe CSV na aba Configurações.</td></tr>';
}

function showDashboardError() {
    const tbody = document.getElementById('alertsTableBody');
    if (tbody) tbody.innerHTML = '<tr><td colspan="5">Erro ao carregar dados. Verifique sua conexão.</td></tr>';
}